...
*(Note: While slightly higher than XGBoost's 18.64, GPR compensates by providing uncertainty bounds, which is crucial for chemical process reliability and phase 2 judging criteria).*
...
```
*This shows the model found a smooth physical trend (Matérn) while accounting for about 5% inherent noise (WhiteKernel) in the dataset.*
...
*   **Test case 2:** Yield = `100.00% ± 16.76%` *(clipped to physical max)*
*   **Test case 3:** Yield = `35.57% ± 27.24%`
*   **Test case 4:** Yield = `100.00% ± 26.98%` *(clipped to physical max)*
*   **Test case 5:** Yield = `0.00% ± 19.09%` *(clipped to physical min)*
